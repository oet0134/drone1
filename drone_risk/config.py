...
RGB = {
    ...
    # 상시 감시(변화 감지) — 기준 화면 대비 '새로 어두워진 곳'을 직접 비교
    "monitor": {
        "ref_dim": 480,        # 비교용 축소 크기(작을수록 빠르고 잡음↓)
        "blur": 2,             # 비교 전 흐림(카메라 노이즈 흡수)
        "diff_thresh": 26,     # 기준보다 이만큼(0~255) 어두워지면 변화로 봄
        "min_region_px": 25,   # 이보다 작은 변화는 잡음으로 무시
        "global_ratio": 0.30,  # 변화 면적이 화면의 이 비율 넘으면 '카메라 이동'으로 처리
    },
}
