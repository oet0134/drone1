...
RGB = {
    ...
    # 상시 감시(변화 감지) — 기준 화면 대비 '새로 진하게 어두워진 곳'을 직접 비교
    "monitor": {
        "ref_dim": 720,        # 비교용 크기(클수록 가는 선도 살아남음)
        "blur": 1,             # 약한 흐림(가는 선 유지하면서 노이즈만 흡수)
        "diff_thresh": 45,     # 이만큼(0~255) '진하게' 어두워져야 변화(그림자·흔들림 배제)
        "min_region_px": 12,   # 가는 선도 잡히게 작게(진하기 조건이 잡음을 막음)
        "global_ratio": 0.28,  # 변화 면적이 이 비율 넘으면 '카메라 이동'으로 처리
        "confirm_frames": 2,   # 이만큼 연속 감지돼야 경보(순간 그림자·흔들림 무시)
    },
}
